...
#raise params.inspect
  ...
  def kewpa3
    @asset = Asset.find(params[:id])
    @asset_loss = AssetLoss.find_by_asset_id(@asset.id)				#retrieve asset loss data & write off if any (hapuskira)
    @asset_disposal = AssetDisposal.find_by_asset_id(@asset.id)		#retrieve asset disposal data if any (pelupusan)
    render :layout => 'report'
  end
