class StudentDisciplineCasesController < ApplicationController
  ...
  def new
     @student_discipline_case = StudentDisciplineCase.new #@asset = Asset.new
  ...
  def create
    @student_discipline_case = StudentDisciplineCase.new(params[:student_discipline_case])#@asset = Asset.new(params[:asset])
    respond_to do |format|
