class Asset < ActiveRecord::Base
  ...
  def save_my_vars
    if assetcode == nil
      ...
      #self.quantity = quantity-cardno.to_i+1 if quantity != nil
      #self.quantity = 1 if quantity == nil
      if assettype == 1			#just added 17dec2012
      	if (receiveddate.year.to_s.last(2)).to_i > (Asset.find_last_by_assettype(1)[:assetcode].split("/")[4]).to_i		#added for new year 
       		self.cardno = "1"
       	else
	 		self.cardno = (Asset.find_last_by_assettype(1)[:assetcode].split("/")[5].to_i + 1).to_s		#just added 17dec2012
		end	  
	  else 
	  	if (receiveddate.year.to_s.last(2)).to_i > (Asset.find_last_by_assettype(2)[:assetcode].split("/")[4]).to_i		#added for new year 
       		self.cardno = "1"
       	end
	 	#self.cardno = ((Asset.find_last_by_assettype(2)[:assetcode].split(/[\/]/)[5].to_i + 1).to_i+(Asset.find_last_by_assettype(2)[:quantity]).to_i-1).to_s
	  end
	  ...
	  if assettype == 1	
	  	...
	  	self.quantity = quantity-self.cardno.to_i+1 if quantity != nil
      	self.quantity = 1 if quantity == nil
	  end
    end
  end
  ...
  #----TEMPORARY HIDE 18 DEC 2012-INDEX PAGE-COZ SEARCH NOT WORKING
  #def self.search(search)
   # if search
...
#------------------------Declaration----------------------------------------------------    
    ...
    def suggested_serial_no
      ...
      if assettype == 1
       md = "H/"
       	#---this IF statement added - for year 2013 assetcode generation - IF SELECTED year is bigger than current year of LAST INSERTED ITEM-> assign to 1
       	if (receiveddate.year.to_s.last(2)).to_i > (Asset.find_last_by_assettype(1)[:assetcode].split("/")[4]).to_i
       		cardno = "1"
       	else
       		cardno = (Asset.find_last_by_assettype(1)[:assetcode].split("/")[5].to_i + 1).to_s	#17Dec2012-force cardno to have this no as it's value instead of taking form value.
      	end
      else
       md = "I/"
        #---this IF statement added - for year 2013 assetcode generation - IF SELECTED year is bigger than current year of LAST INSERTED ITEM-> assign to 1
       	if (receiveddate.year.to_s.last(2)).to_i > (Asset.find_last_by_assettype(2)[:assetcode].split("/")[4]).to_i
       		cardno = "1"
	 	else
       		cardno = ((Asset.find_last_by_assettype(2)[:assetcode].split(/[\/]/)[5].to_i + 1).to_i+(Asset.find_last_by_assettype(2)[:quantity]).to_i-1).to_s	#just added 17dec2012
      	end
      end
      #added this part 17 Dec 2012 - to differentiate serial_no (asset_code) format accordingly
      if assettype == 1
      	st + md + syear + '/' + cardno
      else
      	#quantity = 1 if quantity == nil
      	st + md + syear + '/' + cardno + '-' + quantity.to_s 	#this quantity refer to value of quantity field in the form NOT the actual quantity of an asset.
      end
      #st + md + syear + '/' + cardno	
    end
